...
```js
import 'https://ka-f.webawesome.com/webawesome@3.8.0/components/carousel/carousel.js';
```
...
Using `<wa-carousel>` with SSR

Carousel relies on scroll behaviors to work properly. Carousel will display the first image properly, but will not be interactive.
