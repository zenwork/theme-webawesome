...
```js
import 'https://ka-f.webawesome.com/webawesome@3.8.0/components/include/include.js';
```
...
Using `<wa-include>` with SSR

`<wa-include>` uses fetch, but due to its asynchronous nature similar to [`<wa-icon>`](https://webawesome.com/docs/components/icon), there is no way to get the rendered content on your server.
